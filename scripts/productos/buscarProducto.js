...
async function buscarProductosPorNombre(nombre) {
    try {
        ...
        const response = await fetch(`http://18.206.95.2/productos/producto/${nombre}`);
...
async function buscarProductosPorFecha(fechaInicio, fechaFin) {
    try {
        // Realizar una solicitud al servidor con el término de búsqueda
        const response = await fetch(`http://18.206.95.2/productos/fecha?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`);
...
async function buscarProductosPorNombreYFecha(nombre, fechaInicio, fechaFin) {
    try {
        // Realizar una solicitud al servidor con el término de búsqueda
        const response = await fetch(`http://18.206.95.2/productos/nombreyfecha?nombre=${nombre}&fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`);
